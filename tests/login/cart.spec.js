...
test.describe("Search and Cart", () => {
  ...
  test("Add item to cart from category", async ({ page }) => {
    const login = new LoginPage(page);
    await login.categoryCart();

  });

  test("Add item to cart that is out of stock", async ({ page }) => {
    const login = new LoginPage(page);
    await login.categoryCart();
  });

  test.only("Add item to cart after search", async ({ page }) => {
